class PostAttachmentsController < ApplicationController
  ...
  respond_to do |format|
    if @post_attachment.update(post_attachment_params)
      format.html { redirect_to @post_attachment.blog, notice: 'Image was successfully updated.' }
    end 
  end
end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to post_attachments_url, notice: 'Image was successfully destroyed.' }
  ...
  private
    ...
    def set_post_attachment
      @post_attachment = PostAttachment.friendly.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def post_attachment_params
      params.require(:post_attachment).permit(:blog_id, :avatar)
    end
end
